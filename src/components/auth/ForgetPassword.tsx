...
export default function ForgetPassword({ lang, onSent }: ForgetPasswordProps) {
  ...
  const { mutateAsync, isPending: isLoading } = useMutation({
    ...
    mutationFn: (data: ForgetFormData) =>
      requestPasswordReset(data.email, lang),
    onSuccess: (res) => {
      toast.success(
        t("success:forgotPassword", res?.message || "Reset link sent"),
      );
    },
    onError: (error: unknown) => {
      const err = error as { message?: string };
      toast.error(
        err.message || t("error:forgotPassword", "Failed to send reset email"),
      );
    },
  });
